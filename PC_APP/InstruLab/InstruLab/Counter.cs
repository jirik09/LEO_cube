    ...
    public partial class Counter : Form
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            try
            {
                ...
                else if (req == Message.MsgRequest.COUNTER_REF_DATA)
                {
                    ...
                    double printRefCounter = double.IsInfinity(cntPaint) ? 0 : cntPaint;
                    label_cnt_ref_value.Text = String.Format("{0:n14}", printRefCounter);
                    ...
                    if (cntPaint != 0 && !double.IsInfinity(cntPaint))
                    {
        ...
        private void splitAndSend(UInt32 sampleCount)
        {
            ...
            if ((arr <= 64000) && (psc <= 64000) && (arr > 0) && (psc > 0))
            {
                ...
                device.send(Commands.COUNTER + ":" + Commands.CNT_REF_SAMPLE_COUNT + " ");
                device.send_short((int)sampleCount);
                device.send(";");
        ...
        private void scrollTimeElapseEvent(Object source, ElapsedEventArgs e)
        {
            ...
            switch (timScroll)
            {
                ...
                case CNT_TIMER.TIM_SCROLL_REF1:
                    scrollTimer.Stop();
                    device.send(Commands.COUNTER + ":" + Commands.CNT_REF_SAMPLE_COUNT + " ");
                    device.send_int((int)refSamples);
                    device.send(";");
