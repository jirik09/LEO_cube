...
static void MX_TIM2_ETRorREF_Init(void)
{
	...
	sSlaveConfig.SlaveMode = 0x00010000U;//TIM_SLAVEMODE_GATED;
...
void TIM2_ETRorREF_MspInit(TIM_HandleTypeDef* htim_base)
{
	...
	hdma_tim2_up.Instance = DMA1_Stream7;
...
void TIM_counter_etr_init(void){
	...
	counter.tim4PrphClk = HAL_RCC_GetPLK1Freq();
}
...
//	if (RCC->CFGR3&RCC_TIM2CLK_PLLCLK){
		counter.tim2PrphClk = HAL_RCC_GetPCLK1Freq();
	//}	else {
		//counter.tim2PrphClk = HAL_RCC_GetPCLK1Freq();
	//}
}
...
void TIM_TI_Init(void)
{
	...
	htim2.Instance->SMCR |= 0x00010000U;//TIM_SMCR_SMS_3;
...
bool DMA_TransferComplete(DMA_HandleTypeDef *dmah)
{
	...
	regs = (DMA_Base_Registers *)DMA_CalcBaseAndBitshift(dmah);

	uint32_t dmaIsrReg = regs->ISR;

	if(dmaIsrReg & (uint32_t)(DMA_FLAG_TCIF1_5 << dmah->StreamIndex)){
		/* Clear the transfer complete flag */
		regs->IFCR = DMA_FLAG_TCIF1_5 << dmah->StreamIndex;
