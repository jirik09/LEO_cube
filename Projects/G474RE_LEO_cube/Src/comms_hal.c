...
// Includes ===================================================================
#include "mcu_config.h"      /* Ensure __ALIGN_END undef happens before USB headers */
#include "cmsis_os.h"
#include "comms.h"
#include "comms_hal.h"
#include "adc.h"
...
void commsSendBuff(const uint8_t *buff, uint16_t len){
	#ifdef USE_USB
	if (hUsbDeviceFS.dev_state == USBD_STATE_CONFIGURED){	
		while(CDC_Transmit_FS((uint8_t*)buff,len)!=USBD_OK){ /* cast away const for USB CDC API */
			taskYIELD();
		}
	}else{
	...
	#else
	//UARTsendBuff((char *)buff,len);
	HAL_UART_Transmit(&huart2, (uint8_t*)buff, len, 5000); /* HAL expects non-const */
	#endif
}
